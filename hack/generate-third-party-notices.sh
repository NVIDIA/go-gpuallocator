...
BUNDLED_ASSETS=(
    "vendor/github.com/NVIDIA/go-nvml/pkg/nvml/nvml.h|The NVML API header, copied verbatim into go-nvml from the CUDA \`nvml-dev\` redistributable (the file names its own source and version on line 2). It carries NVIDIA's own license terms, reproduced below, and is **not** covered by go-nvml's Apache-2.0 license."
    "vendor/github.com/NVIDIA/go-nvml/pkg/nvml/cgo_helpers.h|A cgo binding header generated by [c-for-go](https://git.io/c-for-go) as part of go-nvml. Apache-2.0, the same license as the rest of that module; listed here only because go-licenses cannot see non-Go files."
)
...
verify_bundled_assets() {
    ...
    for asset in "${BUNDLED_ASSETS[@]}"; do
        asset_path="${asset%%|*}"
        [[ -f "${asset_path}" ]] \
            || die "declared bundled asset '${asset_path}' does not exist." \
                   "Update BUNDLED_ASSETS in hack/generate-third-party-notices.sh."
    done

    local -a undeclared=()
    local module_path vendored_file filename
    while IFS= read -r module_path; do
        [[ -n "${module_path}" && -d "vendor/${module_path}" ]] || continue
        while IFS= read -r -d '' vendored_file; do
            filename="$(basename "${vendored_file}")"
            case "${filename}" in
                *.go|*.md) continue ;;
            esac
            # LC_ALL=C: under a Turkish locale glibc does not fold I to i.
            if printf '%s' "${filename}" \
                | LC_ALL=C grep -qiE '^(licen[cs]e|notice|copying|copyright|authors|contributors|patents)([-._].*)?$'; then
                continue
            fi
            case " ${BUNDLED_ASSETS[*]%%|*} " in
                *" ${vendored_file} "*) continue ;;
            esac
            undeclared+=("${vendored_file}")
        done < <(find "vendor/${module_path}" -type f -print0 2>/dev/null | LC_ALL=C sort -z)
    done < <(cut -d, -f4 "${INDEX_FILE}" | sed 's/@.*//' | LC_ALL=C sort -u)

    (( ${#undeclared[@]} == 0 )) || die \
        "vendored modules contain non-Go files that go-licenses cannot inspect:" \
        "${undeclared[@]}" \
        "Add each to BUNDLED_ASSETS in hack/generate-third-party-notices.sh with its attribution."
}
    ...
    LC_ALL=C awk '
        {
            ...
            if (substr(trimmed, 1, 2) == "/*") {
                print
                if (index(substr(trimmed, 3), "*/") == 0) in_block_comment = 1
                next
            }
            if (substr(trimmed, 1, 2) == "//") { print; next }
            exit
        }
    ' "$1"
}
...
emit_bundled_assets() {
    local asset asset_path attribution fence
    for asset in "${BUNDLED_ASSETS[@]}"; do
        asset_path="${asset%%|*}"
        attribution="${asset#*|}"
        printf '### %s\n\n' "${asset_path}"
        printf '%s\n\n' "${attribution}"
        fence="$(fence_for "${asset_path}")"
        printf '%stext\n' "${fence}"
        header_comment_for "${asset_path}"
        printf '%s\n' "${fence}"
        echo
    done
}
...
license, and the non-Go files vendored alongside them that carry license terms of
their own. `go-gpuallocator` is a library: the module itself is the unit of
...
## Bundled Non-Go Components

Files vendored inside the modules above that are not Go sources and carry
license terms of their own, shown with the license header each carries upstream.
...
main() {
    ...
    log "Wrote ${OUTPUT} (${package_count} Go packages, ${#BUNDLED_ASSETS[@]} bundled non-Go files)"
}
